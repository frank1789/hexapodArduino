...
void loop() {
  ...
  auto leg = hxm.getLeg(legs[1]);
  for (int i = 1; i < 178; i++) {
    auto str = String("testLeg, for angle: ") + String(i);
    Serial.println(str);
    leg.getComponentLeg("coxa").setServoAngle(i);
    delay(500);
    leg.getComponentLeg("femur").setServoAngle(i);
    delay(500);
    leg.getComponentLeg("tibia").setServoAngle(i);
    delay(500);
  }
}
